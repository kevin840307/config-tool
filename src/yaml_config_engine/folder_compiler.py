    ...
    def apply_manifest(
        ...
        *,
        ...
        if generated_root.is_file():
            return self.apply_compact(source_root, generated_root, output_root, variables=variables)
        manifest_path = generated_root / 'manifest.yaml'
        if not manifest_path.exists() and (generated_root / 'patch.yaml').exists():
            return self.apply_compact(source_root, generated_root / 'patch.yaml', output_root, variables=variables)
